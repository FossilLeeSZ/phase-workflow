...
ChatGPT/MCP planning with Codex execution is optional. In that path, ChatGPT may use the
read-only MCP companion through an available connector path to read bounded project context
and generate a short handoff prompt for Codex. Codex still validates local files, phase
gates, authorization branch, and stop conditions before execution.
...
ChatGPT-side planning uses an already running companion only through an available connector
path. For remote ChatGPT, prefer OpenAI Secure MCP Tunnel when available; keep loopback
endpoint details for same-machine development and smoke tests. Do not let ChatGPT start
Codex, start the local companion, or execute project commands. Do not add a Web UI, database,
...
ChatGPT connector UI, workspace association, and permissions. If the tunnel is not visible or
connector calls fail, check workspace association, Tunnels Read and Tunnels Use, rerun
